...
10]. By default, when validating a data each clause will be tested and must
succeed for the whole validation to succeed (except when there are clauses like
B<min_nok> which requires a certain number of clause I<failures> for the whole
validation to succeed, but this is not often used). Aside from declaring
constraints, clauses can also declare other stuff like default value (the
C<default> clause), store metadata (the C<summary>, C<description>, C<tags>
clauses), etc. Please see type's documentation for the list of known clauses of
each type.
