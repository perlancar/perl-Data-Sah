...
whole validation to succeed (there are exceptions, but it is not important right
now). Aside from declaring constraints, clauses can also declare other stuff
like default value (the C<default> clause), store metadata (the C<summary>,
...
functions. See L</"Clause attributes"> for more information on clause
attributes.
...
EXTRA_STUFFS is a hashref, currently not further specified. It is reserved for
future use.
