...
definition, and all the clause sets will be evaluated. Illustrated:
...
a dotted-separated series of parts, e.g. C<alt.lang.id>. As with clauses, clause
attributes which begin with "_" (underscore) will be ignored by Sah.
...
handler, a custom error message is supplied. LANG is 2-code ISO language code
and can be omitted (to mean the default/fallback language). For example:
...
L<Data::Sah::Compiler::human> compiler to supply description. LANG is a 3-letter
ISO language code and can be omitted (to mean the default/fallback language).
