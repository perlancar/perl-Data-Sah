...
You can specify the same attribute more than once, using the suffix '#2', '#3',
and so on after the attribute name. All the attributes will be evaluated in order
of the suffix number (attribute name without '#' suffix will be assumed to have
'#1' suffix).

 {
   'attr' => value,
   'attr.prop' => prop-value,

   'attr#2' => another-value,
   'attr#2.prop' => prop-value,
   ...
 }
...
Used to specify multiple attribute values, instead of one. Checking will be done
for each value, and attribute checking will succeed if any of the value succeeds.
Example:

 # string must either be all uppercase or all lowercase, no MixED CAse allowed
 [str => {'match.either' => ['^[A-Z]+$', '^[a-z]+$']}]

=head3 all

Used to specify multiple attribute values, instead of one. Checking will be done
for each value, and attribute checking will fail if any of the value fails.
Example:

 # string must contain a letter/digit/underscore as well as some symbols.
 [str => {'match.all' => ['\w', '\W']}]
...
Note that '#2.propname', '#3.propname', and so on is not allowed.
