...
they are both present, attribute hashes from both will be used (with
B<attrs> as the first attribute hash and the rest from
B<attr_hashes>).

This third form allows us to define other schemas inside our schema,
using the B<def> keys, which must be a hashref of schema name and
definition. This is a way to break down or organize a complex schema
into several pieces.
...
An attribute hash is a mapping of attributes and values:
...
An attribute is composed of an optional merge prefix, attribute name,
an optional index, and optional attribute parameters:

 (merge_prefix)? name ("[" index "]")? ("?" params)?
...
attribute hash contains a prefix (see L<"Attribute merge prefix">),
merging will occur (see L<"Merging of attribute hashes">).
...
underscore will be ignored (you can use this to embed extra data for
other purposes).

=head2 Attribute merge prefix

Attribute merge prefix is one of these characters:
...
See L<Data::ModeMerge> to learn more the details of the merging.

=head2 Attribute index

Attribute name can be followed by an index, which is an integer from 1
and up, enclosed by braces. They are used if you want to specify more
than one attributes with the same name. Order of evaluation will start
from the smallest index. If index is not specified, it is assumed to
be 1. Ex:

 [str => {
   'match'    => qr/foo/, # will be evaluated first (index is 1)
   'match[2]' => qr/bar/, # second
   'match[3]' => qr/baz/, # third
 }]
...
allow special characters:
 ...
 { "attr?param1=p1value&param2=p2value" => attr_value, ... }

But there is also an alternate "split" syntax to specify parameters:
...
contains an unknown attribute parameter name. But any parameter names
starting with _ will be ignored.

=head3 errlevel

Valid value: C<error>, C<warn>. Default is C<error>. Normally, when
attribute checking fails, an error is generated and the whole schema
fails to validate. If errlevel is set to C<warn>, however, this only
generates a warning and does not cause the validation to fail.

Example:

 [str=>{minlen                 => 4,
       "minlen?errlevel=warn"  => 8}]
...
This tells the emitter that instead of the default error message from
the type handler, a custom error message is supplied. For example:

 [str=>{"match?errmsg=Must+not+contain+naughty+characters" => qr/[^A-Za-z0-9_-]/}]

Or, using the (much nicer in this case) split syntax:
...
Normally attributes serve as a type constraint (e.g. for type string,
the minlen and maxlen attributes constraint how short/long the string
can be). However there are also some attributes that are special.

=head3 The '' (empty attribute)

This can be used to store parameters that can be used generally. For
example:
...
When validation fails, the generic error message will be used.
...
=head3 Merging of attribute parameters

Attribute parameters also get merged much like the attributes
themselves. Unless specified otherwise, the merge mode of the
attribute will define the merge mode of the params. Example:

 [t => {attr => 1, "attr.param1" => 2, "attr[2]?param1=2" => 3},
       {"+attr" => 10, "attr.param1" => 20, "attr[2]param1=3" => 30},]

The attributes will be merged like this:
...
While the attribute parameters themselves are merged like this. For
attr:
...
While attr[2]'s parameters are not merged because the attribute itself
is replaced.

(XXX: how to specify merging mode for attribute parameters)
