...
=head3 Finding out the schema for an attribute's argument

Attribute argument is specified using schema too. If you want to find out the
schema for int's 'mod' attribute: XXX.

Under the hood, the schema is stored in the attrarg_ATTR() method.

=head3 Listing all known functions

 dd sort keys %{ $ds->func_names };
...
See documentation in Data::Schema::Func::* (e.g. in L<Data::Schema::Func::Std>)
to find out more about each function.

=head3 Finding out the schema for a function's arguments

Function arguments are specified using schemas too. If you want to find out the
schemas for the arguments of the re_replace() function, XXX.

Under the hood, the schema is stored in the funcargs_ATTR() method.


=head2 Extending type

See L<Data::Schema::Manual::ExtendingType>

=head2 Adding new type

See L<Data::Schema::Manual::AddingNewType>
