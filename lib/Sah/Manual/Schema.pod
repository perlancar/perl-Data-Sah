...
A B<schema> is essentially a type, which states a set of valid values. Sah
defines several basic types like "bool" ("boolean"), "int" ("integer"), "str"
("string"), "array", "hash", and a few others. Please see Sah::Schema::Type::*
...
B<Clause attributes>. Aside from having a value (like in C<"divisible_by" => 2>
clause, "divisible_by" is the clause name and 2 is the clause value), a clause
can also have attributes. Clause attributes are used for various function.
...
restrict the set of valid values). See L</"SUBSCHEMA"> for more information on
declaring subschemas locally, and L</"CLAUSES"> on how clauses are merged.


=head2 GENERAL STRUCTURE

Sah schema is just a normal data structure:
...
it in the B<clauses> key instead:
...
attributes. See type documentation (e.g. L<Sah::Type::Hash> for hash type) for
...
L<Sah::Emitter::Human> emitter to supply description. LANG is a 2-code language
...
In practice, using prefix symbols or empty strings as hash keys are in general
not very common, so you don't have to worry to much about it.
...
For the list of functions supported, see L<Sah::Spec::Func::Std>. To add a new
Sah function from Perl, see L<Sah::Manual::Function>.
