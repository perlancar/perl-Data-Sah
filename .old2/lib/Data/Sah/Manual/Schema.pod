...
Data::Sah::Type::* for the complete list.

A type can have B<clauses>, which mostly declare constraints. Examples include
"min" and "max" clauses to declare minimum and maximum values, or str's "match"
clause to specify a regex pattern that value must match. Clauses can also
declare other stuff like default value (the "default" clause), etc. Please see
type's documentation for the list of known clauses of each type. Below are
several schemas, each specifying a basic type and zero or more clauses:
 ...
 [int => {div_by=>2}, {div_by=>3}]

 # a leap year: divisible by 4 && (not divisible by 100 unless divisible by 400)
...
B<Clause attribute>. Aside from having a value (like in C<"div_by" => 2> clause,
"div_by" is the clause name and 2 is the clause value), a clause can also have
attributes. Clause attributes are used for various function. See L</"CLAUSE">
for more information on clause attributes.

B<Subschema>. You can define some schema as a new type and then form a new
schema based on the previous schema along with some additional clauses. This is
very much like subtyping. The difference in Sah is that a subschema can be
defined locally (only known inside the declaring schema) and that a subschema
can actually replace and I<remove> clauses (i.e. relax instead of just further
...
If you don't have any subschema definition, which is very common, you can use
the array form to save some typing:
...
You can even omit the hash to further save typing a couple of curly braces (but
this will limit you to one clause set):
...
handler, a custom error message is supplied. LANG is 2-code language code and
can be omitted (to mean the default/fallback language). For example:

 [str=>{match              => qr/[^A-Za-z0-9_-]/,
        'match:err_msg'    => 'Must not contain naughty characters',
        'match:err_msg.id' => 'Tidak boleh mengandung karakter aneh-aneh',
 }]
...
L<Data::Sah::Compiler::human> compiler to supply description. LANG is a 2-code
language code and can be omitted (to mean the default/fallback language).
...
remove keys from the left side. This allows schema definition to not only add
...
For the list of functions supported, see L<Data::Sah::Spec::Func::Std>. To add a
new Sah function from Perl, see L<Data::Sah::Manual::Function>.
