...
Alternatively, expression can also be specified in other attribute's value when
that attribute's name has 'expr' property (or its shortcut '='), e.g.:
...
functions, enabling more complex schema to be defined, for example:
 ...
 ['array*' => {len=>2, elements => [
   ['str*', {match => '^\w+$'}],
   ['str*', {'match:expr' => '${../../0/attr_hashes/0/match}',
